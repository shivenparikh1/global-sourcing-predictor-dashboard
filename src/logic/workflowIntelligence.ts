    ...
    explanation:
      band === "high"
        ...
        : band === "medium"
          ...
          : "Confidence is low because the app is missing core sourcing inputs needed for a reliable recommendation.",
    nextBestAction,
...
export const buildWorkflowStages = (scenario: Scenario, prediction: PredictionResult, intake: IntakeState): WorkflowStage[] => {
  ...
  return [
    ...
    { id: "recommendation", title: "Generate Recommendation", status: stageStatus(prediction.confidenceScore >= 60 && activeRoutes.length > 0), requiredInputs: ["Data Confidence", "Scenario Comparison", "Tradeoff Explanation"], missingInputs: prediction.confidenceScore >= 60 ? [] : prediction.missingDataFields.slice(0, 4), whyItMatters: "A recommendation turns analysis into an approve, revise, or reject decision.", nextBestAction: "Generate recommendation after core data quality improves.", actionLabel: "Open Recommendations", action: "recommendations" },
...
export const buildRecommendationExplanation = (scenario: Scenario, prediction: PredictionResult, recommendation: Recommendation, strategy: SourcingStrategy): RecommendationExplanation => {
  ...
  return {
    ...
    serviceReasoning: prediction.serviceLevel > 0 ? `Modeled service level is ${prediction.serviceLevel.toFixed(1)}%, influenced by reliability, safety stock, route delay, and supplier diversification.` : "Service reasoning is pending reliability, demand, route, and inventory inputs.",
