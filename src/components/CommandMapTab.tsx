...
function ScorecardSidebar({ prediction, hasPrediction }: { prediction: PredictionResult; hasPrediction: boolean }) {
  const cards = useMemo(
    () => [
      ...
      { label: "Service", value: hasPrediction ? pct(prediction.serviceLevel) : "Incomplete", status: hasPrediction ? "Modeled" : "Need demand", tone: hasPrediction && prediction.serviceLevel >= 88 ? "good" : "warn", icon: ShieldCheck },
